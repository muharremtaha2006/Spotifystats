...
# Geçerli renk isimleri
VALID_COLOR_NAMES = {
    "black", "white", "red", "green", "blue", "yellow", "purple", "pink", "gray", "orange", 
    "cyan", "magenta", "brown", "lime", "gold", "silver", "beige", "indigo", "violet"
}
...
def apply_theme_and_background(root, settings):
    ...
    background_value = settings.get("background_value", "black")

    if background_type == "color":
        if background_value.lower() in VALID_COLOR_NAMES or background_value.startswith("#"):
            root.configure(bg=background_value)
        else:
            messagebox.showwarning(
                "Geçersiz Renk",
                f"'{background_value}' geçersiz bir renk. Varsayılan siyah arka plan ayarlandı."
            )
            root.configure(bg="black")
    elif background_type == "image":
        try:
            img = Image.open(background_value)
...
# Ana Ekranı Yenileme
def reload_main_interface(root, settings):
    apply_theme_and_background(root, settings)
...
def settings_menu(root):
    ...
    def update_settings():
        ...
        reload_main_interface(root, settings)

    def choose_background_color():
        color_code = colorchooser.askcolor(title="Renk Seçin")[1]
        if color_code:
            bg_entry.delete(0, tk.END)
            bg_entry.insert(0, color_code)

    def choose_image():
        file_path = filedialog.askopenfilename(filetypes=[("Görüntü Dosyaları", "*.png;*.jpg;*.jpeg")])
        if file_path:
            bg_entry.delete(0, tk.END)
            bg_entry.insert(0, file_path)
    ...
    bg_color_button = ctk.CTkButton(settings_frame, text="Renk Paleti", command=choose_background_color)
    bg_color_button.pack(pady=5)

    bg_image_button = ctk.CTkButton(settings_frame, text="Resim Seç", command=choose_image)
    bg_image_button.pack(pady=5)
